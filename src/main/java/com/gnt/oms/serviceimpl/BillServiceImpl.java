...
public class BillServiceImpl implements BillService {
        ...
        try {
            ...
            if (validateRequestMap(requestMap)) {
                if (requestMap.containsKey("isGenerate") && !(Boolean) requestMap.get("isGenerate")) {
                    ...
                    fileName = OMSUtil.getUUId();
                    requestMap.put("uuid", fileName);
                    insertBill(requestMap);
                }

                String data = "Name: " + requestMap.get("name") + "\n";
                data = data + "Contact No:" + requestMap.get("contactNo") + "\n";
                data = data + "Email:" + requestMap.get("email") + "\n";
                data = data + "Payment Method:" + requestMap.get("paymentMethod") + "\n";

                Document document = new Document();
                PdfWriter.getInstance(document,
                        new FileOutputStream(OMSConstants.STORE_LOCATION + "\\" + fileName + ".pdf"));
                document.open();
                setRectangleInPdf(document);

                Paragraph chunk = new Paragraph("Order Management System", getFont("Header"));
                ...
                JSONArray jsonArray = OMSUtil.getJsonArrayFromString((String) requestMap.get("productDetails"));
                for (int i = 0; i < jsonArray.length(); i++) {
                    addRow(table, OMSUtil.getMapFromJson(jsonArray.getString(i)));
                }
                document.add(table);
                ...
                document.add(footer);
                document.close();
                return new ResponseEntity<>("{ \"vvid\" : \"" + fileName + "\"}", HttpStatus.OK);

            } else {
                return OMSUtil.getResponseEntity("Required data not found", HttpStatus.BAD_REQUEST);
            }
    ...
    private void insertBill(Map<String, Object> requestMap) {
        try {
            ...
            bill.setContactNo((String) requestMap.get("contactNo"));
    ...
    private boolean validateRequestMap(Map<String, Object> requestMap) {
        ...
        if (requestMap.containsKey("name") && requestMap.containsKey("contactNo") && requestMap.containsKey("email")
    ...
    public ResponseEntity<byte[]> getPdf(Map<String, Object> requestMap) {
        ...
        try {
            ...
            String filePath = OMSConstants.STORE_LOCATION + "\\" + (String) requestMap.get("uuid") + ".pdf";
            if (OMSUtil.isFileExists(filePath)) {
    ...
    public ResponseEntity<String> deleteBill(Integer billId) {
        try {
            ...
            if (!optional.isEmpty()) {
                ...
                return OMSUtil.getResponseEntity("Bill deleted successfully", HttpStatus.OK);
            } else {
                return OMSUtil.getResponseEntity("Bill Id not exists", HttpStatus.OK);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
